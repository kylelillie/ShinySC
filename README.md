...
For exaple, if you want to download the most recent data for Alberta from the table "Building permits, by type of structure and type of work" and read it into Pandas, all you need to do is:

    url = ShinySC.make_url(34100292,filters={'Geography:['Alberta']})
    df = pd.read_csv(url)
...
Other options include specifying the number of recent periods (integer), data series start date (YYYY-MM-DD) and/or an end date (YYYY-MM-DD). For example:

Building permits data for the year 2020:

    ShinySC.make_url(34100292,start='2020-01-01',end='2020-12-31')

The most recent month of building permits:

    ShinySC.make_url(34100292,periods=1)